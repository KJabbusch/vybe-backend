use rspotify::{
    model::{Country, Market, SearchType, TrackId, Id, RecommendationsAttribute, ArtistId, UserId, PlaylistId},
    prelude::*,
...
fn main() {
    ...
    let playlist_id = PlaylistId::from_id("5UJ0JopBcT5LR7maOIrpda").unwrap();
    let user_id = UserId::from_id("localhotdad").unwrap();
    let kris = spotify.user(&user_id);
    // println!("{:#?}", kris);
